...
def rrc_pulse(t, beta, Tc):
    ...
    for i in range(len(t)):
        if t[i] == 0:
            ...
            numerator = (np.sin(np.pi * t[i] * (1 - beta) / Tc) + 
                         4 * beta * t[i] / Tc * np.cos(np.pi * t[i] * (1 + beta) / Tc))
...
def generate_s_t(t, T, L, x, Lb, Tc, beta):
    ...
    for l in range(1, L+1):  # The range of summation
...
def channel_response(t,beta, Tc, v,sigma_0, tau_0, tau_c, k_c, lambda_c, N_c):
    ...
    beta_0 = np.sqrt(sigma_0 / 2) * (np.random.randn() + 1j * np.random.randn()) #β0 ∼ CN(0,σ2)
    
    beta_c = (np.sqrt(lambda_c * (np.random.weibull(k_c, N_c)) / 2) * np.exp(1j * 2 * np.pi * np.random.rand(N_c))) # the amp of Nc clutters which are independent with uniform phases and Weibull absolute values
...
# Create signals----------
h_t = channel_response(config.t, config.beta, config.Tc, config.v, config.sigma_0, 
                       config.tau_0, config.tau_c, config.k_c, config.lambda_c, config.Nc)
print("length of h(t):", len(h_t))

t, s_t = generate_s_t(config.t, config.T, config.L, config.x, config.Lb, config.Tc, config.beta)
...
z = np.random.normal(0, np.sqrt(N0 * B), num_noise_samples) + 1j * np.random.normal(0, np.sqrt(N0 * B), num_noise_samples)
y_noisy = y_t + z 


# Sampling y(t) to obtain yi based on Tc----
num_samples_to_take = 2 * config.Lb * config.L  # total number of samples
sample_indexes = np.arange(1, num_samples_to_take + 1) * config.Tc  # array of sampling indexes with Tc intervals
yi_indexes = (sample_indexes / (config.T / config.sps)).astype(int) # convert sample indexes to the corresponding indexes in y_t 
...
print("Samples for first slot (y1):", yl[0]) # just for test and checking the first slot
...
plt.bar(x_positions - bar_width / 2, config.x, width=bar_width, color='red', alpha=0.8, label='Bit Vector x')# bar chart for bit vector x
