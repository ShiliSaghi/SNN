...
Lb = 1           # Bandwidth factor, in paper Lb ∈ [1,6] ???
Tc = 1.0         # chip time, symbol duration ???
T = 2 * Lb * Tc  # Time slot duration: 2*1
beta = 0.25      # roll-off factor-(rrc) ???
sps = 100        # samples per symbol  ???
t = np.linspace(0, L * T, L * 2 * Lb * sps) # Time vector: MaxTime: L * T = 80 * 2 = 160, No of samples: L * 2 * Lb * sps = 80*2*1*100=16000
...
tau_c = np.random.uniform(0, 4 * Tc, Nc)  # clutter delays uniformly distributed between 0 and 4Tc
