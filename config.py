...
Tc = 1                  # chip time, symbol duration ???
T = 2 * Lb * Tc         # Time slot duration: 2
beta = 0.25             # roll-off factor-(rrc) ???
t = np.linspace(0, L * T, L * 2 * Lb)       # Time vector: MaxTime: L * T = 80 * 1 = 80, No of samples: L * 2 * Lb = 80*2*2=320
...
tau_c = np.random.uniform(0, 4 * Tc, Nc)  #np.random.choice(np.arange(0, int(4 * Tc)+1), size=Nc, replace=False) #np.random.uniform(0, 4 * Tc, Nc)  # clutter delays uniformly distributed between 0 and 4Tc
